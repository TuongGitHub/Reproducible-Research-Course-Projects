...
```{r, echo=TRUE }
activity <- rawdata[which(!is.na(rawdata$steps)),]
...
```{r, echo=TRUE }
timeseries <- aggregate(x = activity$steps, by = list(activity$interval), mean, na.rm = TRUE)
...
```{r, echo=TRUE}
intervalmeansteps <- aggregate(x = activity$steps, by = list(activity$interval), mean, na.rm = TRUE) 
...
```{r, echo=TRUE}
filledactivity <- merge(x=rawdata,y=intervalmeansteps,by='interval') %>%
  mutate(steps = ifelse(is.na(steps),mean_steps,steps)) %>%
  select(date,interval,steps)
filledactivity <- filledactivity[order(filledactivity$date,filledactivity$interval),]
head(filledactivity)
```
...
```{r, echo=TRUE }
daily <- aggregate(x = filledactivity$steps, by = list(filledactivity $date), sum, na.rm = TRUE)
colnames(daily) <- c("date","steps")
hist(daily$steps,breaks = 10, 
     main = "Histogram of the total number of steps taken daily",
     xlab = 'number of steps')
...
```{r, echo=TRUE }
df <-  mutate(filledactivity,weekday = weekdays(as.Date(date),abbreviate = TRUE))
...
xt <- xtable(summary(fit),comment = FALSE)
print(xt, type = "html")
```
